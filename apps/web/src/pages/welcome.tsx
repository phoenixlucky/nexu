        ...
        <div className="relative flex flex-1 items-center justify-center overflow-hidden bg-[#f7f5ef] px-5 py-8 text-text-primary sm:px-8 lg:px-10">
          ...
          <div className="relative z-10 w-full max-w-[620px]">
            <nav className="mb-8 flex items-center justify-between lg:hidden">
              <button onClick={() => navigate('/')} className="flex items-center cursor-pointer text-accent">
                <img src="/logo.svg" alt="nexu" className="h-5 w-auto object-contain" />
              </button>
              <div className="flex items-center gap-3">
                <div className="text-[11px] uppercase tracking-[0.16em] text-text-muted">{t('welcome.mobileLabel')}</div>
                ...
                <div className="rounded-[32px] border border-black/10 bg-white/88 p-5 shadow-[0_18px_60px_rgba(0,0,0,0.08)] backdrop-blur-sm sm:p-7">
                          ...
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex items-center gap-3 min-w-0">
                              <div className={`flex h-11 w-11 items-center justify-center rounded-2xl shrink-0 ${
                                option.tone === 'primary' ? 'bg-white/[0.08] text-white' : 'bg-white text-text-primary border border-black/8'
                              }`}>
                                <option.icon size={18} />
                              </div>
                              <div className={`text-[22px] leading-none tracking-tight ${
                                option.tone === 'primary' ? 'text-white' : 'text-[#1b1b19]'
                              }`} style={{ fontFamily: 'Georgia, Times New Roman, serif' }}>
                                {option.title}
                              </div>
                            </div>
                            <span className={`rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] shrink-0 ${
                              option.tone === 'primary' ? 'bg-white/[0.08] text-white/75' : 'border border-black/10 bg-white/70 text-text-secondary'
                            }`}>
                              {option.badge}
                            </span>
                          </div>
                  ...
                  <FadeIn delay={380}>
                    <div className="mt-5 flex items-center justify-center gap-4 border-t border-black/8 pt-5 text-[12px] text-text-muted">
                      <button onClick={() => navigate('/terms')} className="cursor-pointer transition-colors hover:text-text-secondary">
                        {t('auth.terms')}
                      </button>
                      <span className="select-none text-border-hover">·</span>
                      <button onClick={() => navigate('/privacy')} className="cursor-pointer transition-colors hover:text-text-secondary">
                        {t('auth.privacy')}
                      </button>
                    </div>
                  </FadeIn>
