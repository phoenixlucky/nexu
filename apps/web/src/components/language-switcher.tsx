...
export function LanguageSwitcher({ variant = 'light', size = 'sm' }: Props) {
  ...
  const base = size === 'sm' ? 'h-11 min-w-[162px] px-3.5 text-[13px]' : 'h-12 min-w-[176px] px-4 text-[14px]';
  const optionBase = size === 'sm' ? 'h-10 px-4 text-[13px]' : 'h-11 px-4 text-[14px]';
  ...
  const colors = {
    light: {
      trigger: 'border border-white/8 bg-[#222227]/92 text-white shadow-[0_8px_24px_rgba(0,0,0,0.18)] backdrop-blur-md hover:bg-[#27272d]',
    ...
    dark: {
      trigger: 'border border-black/8 bg-white/92 text-[#18181b] shadow-[0_8px_24px_rgba(0,0,0,0.08)] backdrop-blur-md hover:bg-white',
    ...
    muted: {
      trigger: 'border border-border bg-surface-1 text-text-primary shadow-[0_8px_24px_rgba(0,0,0,0.05)] hover:bg-surface-0',
  ...
  return (
    <div ref={rootRef} className="relative inline-flex">
      ...
      >
        ...
        <ChevronDown size={size === 'sm' ? 16 : 17} className={`shrink-0 transition-transform ${open ? 'rotate-180' : ''}`} />
      </button>
