...
describe("Gateway skills.ts", () => {
  ...
  it("1b. unchanged hash after degraded → restores active status", async () => {
    const state = createRuntimeState();
    const payload = makePayload({ "my-skill": "# Content" }, "abc123");

    // Simulate: first successful poll sets hash
    (fetchJson as ReturnType<typeof vi.fn>).mockResolvedValue(payload);
    await pollLatestSkills(state);
    expect(state.skillsSyncStatus).toBe("active");

    // Simulate: transient failure set status to degraded
    state.skillsSyncStatus = "degraded";

    // Next poll succeeds with same hash — should restore active
    const result = await pollLatestSkills(state);
    expect(result).toBe(false);
    expect(state.skillsSyncStatus).toBe("active");
  });
