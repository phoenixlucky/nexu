...
|------|-----|
| 框架 | Vitest 3.0.7 |
| 测试 DB | `postgresql://nexu:nexu@localhost:5433/nexu_test` |
| 运行全部 | `pnpm test` |
...
路由测试中需要 mock Better-Auth session：

```typescript
// 方案 A: 直接 mock middleware
...
// 方案 B: mock Better-Auth getSession
vi.mock("../auth.ts", () => ({
  auth: {
    api: {
      getSession: vi.fn(() => ({
...
使用真实测试 DB（`nexu_test`），每个 test suite 独立建表 + truncate：
...
test:
  ...
  steps:
    ...
    - run: pnpm test
      env:
        TEST_DATABASE_URL: postgresql://nexu:nexu@localhost:5433/nexu_test
