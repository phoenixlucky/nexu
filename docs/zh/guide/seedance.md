...
> 当前这篇主教程主要面向 Mac 用户。
>
> 如果你使用的是 Windows，请优先查看 [Seedance 2.0 视频生成（Windows 版）](https://docs.nexu.io/zh/guide/seedance-windows)。
...
**截图要求**：截图里需要清楚看到 **仓库名称**、**已经点亮的 Star 状态**，以及 **当前登录的 GitHub 账号**。这三项都会影响审核，尽量一次截全。
...
选一个你平时最常用的渠道，按页面提示配置完成即可。详细步骤可以看 [渠道配置](./channels.md)。
...
**Q: 如果 Seedance 2.0 没有返回正确的视频链接怎么办？**

请将下面整段作为提示词发给 Agent：

> 参照 Libtv Skill 的步骤来检查视频任务是否已经提交，要有 session ID 和 Project ID。
>
> 如果已提交，请检查视频返回结果是不是符合我给你的格式。
>
> 格式参考：<https://libtv-res.liblib.art/sd-gen-save-img/genius_playground/video>
